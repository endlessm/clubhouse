...
class HackyBalls2(Quest):

    TARGET_APP_DBUS_NAME = 'com.endlessm.hackyballs'


    def __init__(self):
        super().__init__('Hacky Balls 2', 'teacher',
                         "Let's perform a new physics experiment!")
    ...
    def step_first(self, step, starting, time_in_step):
        if starting:
            self.show_message("Excellent. Let's start by launching the Hacky Balls app.")

        if Desktop.app_is_running(self.TARGET_APP_DBUS_NAME):
            return self.step_wait_score

        if (time_in_step > 20 and not self._hint0):
            self.show_message("You need to launch the Hacky Balls app on the desktop.")
            self._hint0 = True
    ...
    def step_wait_score(self, step, starting, time_in_step):
        if starting:
            self.show_message("All right, pay attention: We want to have 10 balls of each time. They're all supposed to move constantly but they can't touch any other balls or the walls.")
        ...
        try:
            if not self._initialized:
                ...
                print("Initialized")
            elif self.debug_was_key_pressed() or self._app.get_object_property('view_hack', 'goal2'):
    ...
    def step_reward(self, step, starting, time_in_step):
        if starting:
            self.show_message("Fantastic! That required some interesting hacking.")
