...
class MazePt1(Quest):
    ...
    def __init__(self):
        super().__init__('MazePt1', 'riley')  # super() calls the __init__() which is a python constructor in the parent class .  MazePt1 is the class we are defining rn.   riley is the current character
        self._app = Maze()   # self is the same as C#'s keyword this,  and _ before a var name is the convention when its an internal variable not exposed outside the class.  Maze() calls the constructor of the Maze class, ie - it instantiates it so from now when we refer to _app, we can communciate with the Maze app via the Maze class
	...
	# wait for the user to launch the app
        ...
        return step_play_level  # return moves the code along to the next step. when you return this step, the parent class runs it
	...
	# Riley says MAZEPT1_MANUAL1 which hasMAZEPT1_MANUAL1_HINT1 _HINT2 and _HINT3
		...
		# and it automatically knows to loop through the _HINT# variations and then asks "what is my goal?" then loops back to start
