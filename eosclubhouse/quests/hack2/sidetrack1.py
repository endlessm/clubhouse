...
class Sidetrack1(Quest):
    ...
    def step_begin(self):
        ...
        if self.app.get_js_property('highestAchievedLevel') > 1:
    ...
    def step_play_level(self, level_changed, level_succeed=None):
        ...
        elif current_level != self.app.get_js_property('currentLevel'):
            level_changed = True
            self._reset_confirmed_messages()
        else:
            # Current level hasn't changed, so either the player
            # completed the level or died:
            level_succeed = self.app.get_js_property('success')
