...
class BlendMonster7(Quest):

    __tags__ = ['pathway:art', 'difficulty:hard']
    __pathway_order__ = 606

    def setup(self):
        self._info_messages = self.get_loop_messages('INFO', start=1)

    def step_begin(self):
        self.deploy_file('monster_smoothie_part_1.blend',
                         '~/BlenderSources/MonsterSmoothieFiles', override=True)
        self.deploy_file('monster_smoothie_part_2.blend',
                         '~/BlenderSources/MonsterSmoothieFiles', override=True)
        self.deploy_file('monster_smoothie_part_3.blend',
                         '~/BlenderSources/MonsterSmoothieFiles', override=True)
        self.deploy_file('monster_smoothie_part_4.blend',
                         '~/BlenderSources/MonsterSmoothieFiles', override=True)
        self.deploy_file('monster_smoothie_part_5.blend',
                         '~/BlenderSources/MonsterSmoothieFiles', override=True)
