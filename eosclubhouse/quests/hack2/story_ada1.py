...
class Story_Ada1(Quest):
    ...
    def step_main_loop(self):
        ...
        #   goto end
        #
        for step in range(1, self.__total_steps__):
            message_id = 'STORY_ADA1_' + str(step)
            if step in self.__question_steps__:
                self.ask_question(message_id)
            else:
                self.wait_confirm(message_id, narrative=True, confirm_label='>')
        return self.step_end

    def ask_question(self, proposal_string):
        # u"\U0001F44D"     /    u"\U0001F44E"
        # U+1F44D for thumbs up, U+1F44E for down
        self.show_message(proposal_string, narrative=True, choices=[
            (u"\U0001F44D", self.ask_question, proposal_string, True),
            (u"\U0001F44E", self.ask_question, proposal_string, False)])

    def display_answer(self, proposal_string, answer):
        if answer:
            self.wait_confirm(proposal_string + '_RPOS', narrative=True, confirm_label='>')
        else:
            self.wait_confirm(proposal_string + '_RNEG', narrative=True, confirm_label='>')
