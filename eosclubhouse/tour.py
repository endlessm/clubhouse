...
#
# Authors:
#       Daniel García Moreno <daniel@endlessm.com>
#
...
class TourServer(GObject.GObject):
    ...
    def _call_method(klass, method, *args, callback=None):
        ...
        def _on_done(proxy, result):
            ...
            try:
                ...
                logger.error('Error calling %s on tour: %s', method, e.message)

            if callback:
                callback(ret)

        klass._get_proxy().call(method, variant,
            Gio.DBusCallFlags.NONE, klass._CALL_TIMEOUT, None, _on_done)
    ...
    @classmethod
    def show_desktop(klass, show, callback=None):
        if show:
            klass._call_method('Overview', 'showApps', callback=callback)
        else:
            klass._call_method('Overview', 'hide', callback=callback)
